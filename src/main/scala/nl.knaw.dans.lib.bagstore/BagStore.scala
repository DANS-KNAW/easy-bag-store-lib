...
trait BagStore {
   ...
   *
   * Although it fetching multiple items to the same target path is not prohibited by the BagIt specification,
   * this condition will lead to a failure. The only way such a bag would be virtually valid is if the
   * conflicting fetch items would all fetch identical files (i.e. with the same payload checksums). This
   * function assumes that conflicting fetch targets are unintentional and will fail early.
   *
...
private[bagstore] class BagStoreImpl(val baseDir: File,
  ...
  override def add(bag: File, optUuid: Option[UUID] = None, move: Boolean = false): Try[BagItem] = {
    ...
    def checkForConflictingFetchItemTargets: Try[Unit] = {
      for {
        inspector <- createBagInspector(bag)
        fetchItems <- inspector.getFetchItems
        conflicts = fetchItems
          .groupBy(_.path)
          .mapValues(_.size)
          .filter { case (path, size) => size > 1 }
        _ = logger.debug(s"conflicts = $conflicts")
        result <- if (conflicts.nonEmpty) Failure(new IllegalArgumentException(s"Conflicting fetch items for paths: ${ conflicts.keys.mkString(", ") }"))
                  else Success(())
      } yield result
    }

    def checkForFetchToExistingFile: Try[Unit] = {
      for {
        inspector <- createBagInspector(bag)
        fetchPaths <- inspector.getFetchItems.map(_.map(_.path).toSet)
        filePaths <- inspector.getBagFiles.map(_.map(f => bag.relativize(f.path)).toSet)
        conflicts = fetchPaths intersect filePaths
        result <- if (conflicts.nonEmpty) Failure(new IllegalArgumentException(s"Fetch item target paths conflicting with existing file(s): ${conflicts.mkString(", ")}"))
             else Success(())
      } yield result
    }
    ...
    for {
      ...
      _ <- checkForConflictingFetchItemTargets
      _ = debug("No conflicting fetch items found")
      _ <- checkForFetchToExistingFile
      _ = debug("No conflicting fetch/files found")
      validity <- isVirtuallyValid(bagToMove)
      _ = debug(s"virtually-valid? $validity")
      _ <- validity match {
        case Right(()) => Success(())
        case Left(problems) =>
