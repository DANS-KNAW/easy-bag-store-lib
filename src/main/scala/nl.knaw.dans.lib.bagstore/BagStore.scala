...
/**
 * Represents a new or existing bag store. If the bag store already exists the onus is on the caller
...
class BagStore(val baseDir: File,
  ...
  def add(bag: File, optUuid: Option[UUID] = None, move: Boolean = false): Try[BagItem] = {
    ...
    def moveBagInto(bag: File, container: File) = Try {
    ...
    for {
      _ <- checkArgs
      staged <- stageBag
      _ <- checkVirtuallyValid(staged)
      uuid <- getUuid
      container <- createContainer(uuid)
      _ <- moveBagInto(staged, container)
